...
The sixth program **autoFind** try to find the desired seed through crossbreeding. For best results, ensure the entire working area is covered with seeds. Once the program finds the desired seed, the **autoStat** will start. Make sure that the desired plant can be obtained from seeds on the working farm.

    autoFind <seed name>

<details><summary>Seed Names</summary>
